...
func (r *clientRepo) Create(ctx context.Context, client *domain.Client) error {
	...
	sqlStatement := "INSERT INTO client(name, surname, birthday, gender, registration_date, address_id) VALUES @clientName, @clientSurname, @clientBithday, @clientGender, @clinetRD, @clientAddressId"
	args := pgx.NamedArgs{
...
func (r *clientRepo) GetAll(ctx context.Context, limit, offset int) ([]domain.Client, error) {
	...
	sqlStatement := "SELECT * FROM client WHERE LIMIT @limit, OFFSET @offset"
	args := pgx.NamedArgs{
		"limit":  limit,
		"offset": offset,
	}
...
func (r *clientRepo) GetByNameAndSurname(ctx context.Context, client domain.Client) ([]domain.Client, error) {
	...
	rows, err := r.db.Query(ctx, sqlStatement, args)
	if errors.Is(err, pgx.ErrNoRows) {
		r.logger.Debug("Client not found", logger.Err(err), "op", op)
		return nil, ErrClientNotFound
	}
